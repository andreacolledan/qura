...
inferWithIndices :: Expr -> TypeDerivation (Type, Index)
-- UNIT
inferWithIndices EUnit = withScope EUnit $ return (TUnit, Number 0)
-- VARIABLE
inferWithIndices e@(EVar id) = withScope e $ do
  typ <- typingContextLookup id
  return (typ, wireCount typ)
-- TUPLE
inferWithIndices e@(ETuple es) = withScope e $ do
  when (length es < 2) $ error "Internal error: tuple with less than 2 elements escaped parser."
  ress <- mapM (withWireCount . inferWithIndices) es
...
-- NIL
inferWithIndices e@(ENil anno) = withScope e $ case anno of
...
-- ABSTRACTION
inferWithIndices e@(EAbs p annotyp e1) = withScope e $ do
  checkWellFormedness annotyp
  (ids, annotyps) <- makePatternBindings p annotyp SizedLists
  ((typ, i), wc) <- withWireCount $ withBoundVariables ids annotyps $ inferWithIndices e1
  return (TArrow annotyp typ i wc, wc)
-- LIFT
inferWithIndices e@(ELift e1) = withScope e $ do
  (typ, i) <- withNonLinearContext $ inferWithIndices e1
  unlessEq i (Number 0) $ throwLocalError $ UnexpectedWidthAnnotation e1 (Number 0) i
  return (TBang typ, Number 0)
-- CONS
inferWithIndices e@(ECons e1 e2) = withScope e $ do
  (typ1, i1) <- inferWithIndices e1
  ((TList j typ1', i2), wc) <- withWireCount $ inferWithIndices e2
...
-- FOLD
inferWithIndices e@(EFold e1 e2 e3) = withScope e $ do
  -- naming conventions are not satisfied here because the rule is HARD to parse
  (steptyp@(TBang (TIForall id (TArrow (TTensor [acctyp, elemtyp]) acctyp' stepwidth o1) o2 o3)), i1) <- inferWithIndices e1
  ...
  ((acctyp'', i2), wc1) <- withWireCount $ inferWithIndices e2
  unlessSubtype acctyp'' (isub (Number 0) id acctyp) $ throwLocalError $ UnfoldableAccumulator e2 acctyp''
  ((argtyp@(TList arglen elemtyp'), i3), wc2) <- withWireCount $ inferWithIndices e3
...
inferWithIndices e@(EApp e1 e2) = withScope e $ do
  (TArrow annotyp typ3 j1 j2, i1) <- inferWithIndices e1
  ((typ2, i2), wc) <- withWireCount $ inferWithIndices e2
  ...
  return (typ3, k)
-- APPLY (CIRCUITS)
inferWithIndices e@(EApply e1 e2) = withScope e $ do
  (TCirc j intyp outtyp, i1) <- inferWithIndices e1
  ((typ2, i2), wc) <- withWireCount $ inferWithIndices e2
  ...
  return (outtyp, k)
-- BOX
inferWithIndices e@(EBox anno e1) = withScope e $ case anno of
  Just annotyp -> do
    (typ1@(TBang (TArrow typ2 typ3 j1 _)), i) <- inferWithIndices e1
...
-- LET-IN
inferWithIndices e@(ELet p e1 e2) = withScope e $ do
  (typ1, i1) <- inferWithIndices e1
  (ids, typs) <- makePatternBindings p typ1 SizedLists
  ((typ2, i2), wc) <- withWireCount $ withBoundVariables ids typs $ inferWithIndices e2
  -- max(i1 + wires in e2, i2):
  let k = Max (Plus i1 wc) i2
  return (typ2, k)
-- ANNOTATION
inferWithIndices e@(EAnno e1 annotyp) = withScope e $ do
  checkWellFormedness annotyp
  (typ, i) <- inferWithIndices e1
  unlessSubtype typ annotyp $ do
    actual <- runSimplifyType typ
    throwLocalError $ UnexpectedType e1 annotyp actual
  return (annotyp, i)
-- FORCE
inferWithIndices e@(EForce e1) = withScope e $ do
  (TBang typ', i) <- inferWithIndices e1
  return (typ', i)
-- INDEX ABSTRACTION
inferWithIndices e@(EIAbs id e1) = withScope e $ do
  ((typ, i), wc) <- withWireCount $ withBoundIndexVariable id $ inferWithIndices e1
  return (TIForall id typ i wc, wc)
-- INDEX APPLICATION
inferWithIndices e@(EIApp e1 g) = withScope e $ do
  (TIForall id typ2 j1 _, i) <- inferWithIndices e1
  checkWellFormedness g
  return (isub g id typ2, Max i (isub g id j1))
-- CONSTANTS
inferWithIndices e@(EConst c) = withScope e $ return (typeOf c, Number 0)
-- ASSUMPTION (unsafe)
inferWithIndices e@(EAssume e1 annotyp) = withScope e $ do
  checkWellFormedness annotyp
  (_, i) <- inferWithIndices e1
  return (annotyp, i)

--- EXPORTED WRAPPER FUNCTIONS ---------------------------------------------------------------

-- | @runTypeInferenceWith env e@ runs the whole type inference pipeline on expression @e@,
...
runAnalysisWith env e = runExceptT $ do
  ...
  ((typ, i), remaining) <- runTypeDerivation (inferWithIndices e') env
...
-- | @runTypeInference e sh@ runs the whole type inference pipeline on expression @e@,
